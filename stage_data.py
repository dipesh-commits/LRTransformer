from lrtransformer import loadFromH5, savePCD
import h5py
import itertools
import sys
import numpy as np
...
#for AREA in ['synthetic_train','synthetic_test']:
	...
	# elif DATA_AREAS in ['s3dis', 'scannet', 'kitti_val']:
	elif AREA in ['s3dis', 'scannet', 'kitti_val']:
		all_points,all_obj_id,all_cls_id = loadFromH5('data/%s.h5' % AREA)
		# all_points,all_obj_id,all_cls_id = loadFromH5('data/%s_area%s.h5' % (DATA_AREAS, AREA))	
	elif AREA=='kitti_train':
		all_points,all_obj_id,all_cls_id = loadFromH5('data/%s_%02d.h5' % (AREA, SEED))
	elif DATA_AREAS[0] in ['shapenet']:
		all_points,all_obj_id,all_cls_id = loadFromH5('data/shapenet_part_seg/region_grow_data/%s_train_%s.h5' % (DATA_AREAS[0], AREA))
	else:
...
#	for room_id in [0]:
		...
		# points = np.hstack((xyz, room_coordinates, normals, curvatures.reshape(-1,1))).astype(np.float32)
...
#		h5_fout = h5py.File('data/small_area%s.h5'%(AREA),'w')
		...
		h5_fout = h5py.File('data/more_seed/s3dis/seed%d_area%s.h5'%(SEED,AREA),'w')
