...
class FurthestSampling(Function):
    ...
    def forward(ctx, xyz, offset, new_offset):
        ...
        # logger.error(b)
        # logger.error(n_max)
        # logger.error(xyz.dtype)
        # logger.error(offset.dtype)
        # logger.error(new_offset.dtype)
        # logger.error(tmp.dtype)
        # logger.error(idx.dtype)
        pointops_cuda.furthestsampling_cuda(b, n_max, xyz, offset, new_offset, tmp, idx)
        logger.info("Passed from furthesampling")
        del tmp
        return idx
...
class KNNQuery(Function):
    ...
    def forward(ctx, nsample, xyz, new_xyz, offset, new_offset):
        ...
        logger.error(m)
        logger.error(nsample)
        logger.error(xyz.dtype)
        logger.error(new_xyz.dtype)
        logger.error(offset.dtype)
        logger.error(new_offset.dtype)
        logger.error(idx.dtype)
        logger.error(dist2.dtype)
