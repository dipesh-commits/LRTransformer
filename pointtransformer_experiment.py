    ...
    def forward(self, pxo) -> torch.Tensor:
        ...
        for i, layer in enumerate(self.linear_p):
            if i==1:
                ...
                p_r = layer(p_r).reshape(p_r.shape[0], p_r.shape[1], 512, 16).permute(0,2,3,1).contiguous()
        ...
        for i, layer in enumerate(self.linear_w):
            if i%3 == 0:
                 ...
                 w = layer(w).reshape(w.shape[0], w.shape[1], 512,16).permute(0,2,3,1).contiguous()
...
class PointTransformerSeg(nn.Module):
    ...
    def __init__(self, block, blocks, planes, c=13, k=0, inlier_first=False, neighbor_first=False, inlier=False, neighbor=False, second_last_block=False, last_block=False):
        ...
        self.second_last_block = second_last_block
    ...
    def _make_enc(self, block, planes, blocks, share_planes=8, stride=1, nsample=16):
        ...
        for _ in range(blocks):
            ...
            if self.inlier_first or self.neighbor_first:
                ...
                if self.second_last_block:
                     self.in_planes = 2304
                     self.second_last_block = False

                if self.last_block:
                    # it is hard coded. need to make it dynamic
                    self.in_planes = 1152    # for last block we have a total of features from (inlier branch from second block+neighbor branch from second block+output from first block) as input after pooling and concat
    ...
    def forward(self, pxo):
        ...
        for _, enc in enumerate(self.encoders):
            p,x = enc([p,x])      # p means xyz, x means features   (b,n,c)
...
class LRGPointTransformer(nn.Module):
    def __init__(self, batch, seq_len, num_inlier, num_neighbor, feature_size):
        super(LRGPointTransformer, self).__init__()
        self.batch = batch
        ...
        # added for new experiment
        self.mid_inliner = PointTransformerSeg(PointTransformerBlock, [1,1], [128,256], inlier=True)
        self.mid_neighbor = PointTransformerSeg(PointTransformerBlock, [1,1], [128,256], neighbor=True)

        self.block2_inliner = PointTransformerSeg(PointTransformerBlock, [1,1], [512,1024], inlier=True)
        self.block2_neighbor = PointTransformerSeg(PointTransformerBlock, [1,1], [512,1024], neighbor=True)

        # added for new experiment
        self.second_last_inlier = PointTransformerSeg(PointTransformerBlock, [1,1], [1024, 512], inlier=True, second_last_block=True)
        self.second_last_neighbor = PointTransformerSeg(PointTransformerBlock, [1,1], [1024, 512], neighbor=True, second_last_block=True)

        self.block3_inliner = PointTransformerSeg(PointTransformerBlock, [1,1], [256,128],  k=2, inlier=True, last_block=True)
        self.block3_neighbor = PointTransformerSeg(PointTransformerBlock, [1,1], [256,128], k=2, neighbor=True, last_block=True)
    ...
    def forward(self, x):
        ...
        # added for new experiment
        _, to_remove_feat_mid = self.mid_inliner(to_remove_feat_b1)
        _, to_add_feat_mid = self.mid_neighbor(to_add_feat_b1)

    
        _, to_remove_feat_b2 = self.block2_inliner(to_remove_feat_mid)
        _, to_add_feat_b2 = self.block2_neighbor(to_add_feat_mid)

        

        feature_pool = torch.cat((
            torch.amax(to_remove_feat_b2,dim=1,keepdim=True),
            torch.amax(to_add_feat_b2,dim=1,keepdim=True)
        ),dim=-1)

        feature_pool = torch.broadcast_to(feature_pool, (feature_pool.shape[0], self.num_inlier, feature_pool.shape[2]))
        

        # added for new experiment

        # concat features from mid and feature pool output
        to_remove_feat_mid = torch.cat((feature_pool, to_remove_feat_mid),dim=-1)
        to_add_feat_mid = torch.cat((feature_pool, to_add_feat_mid), dim=-1)

        # go through second last layer
        _, to_remove_second_last = self.second_last_inlier(to_remove_feat_mid)
        _, to_add_second_last = self.second_last_neighbor(to_add_feat_mid)
        
        # concat features from second last layer
        second_feature_pool = torch.cat((
            torch.amax(to_remove_second_last,dim=1,keepdim=True),
            torch.amax(to_add_second_last,dim=1,keepdim=True)
        ),dim=-1)

        # broadcasting
        second_feature_pool = torch.broadcast_to(second_feature_pool, (second_feature_pool.shape[0], self.num_inlier, second_feature_pool.shape[2]))

        # concat features from block1 and second feature pool

        to_remove_feat_b1 = torch.cat((second_feature_pool, to_remove_feat_b1),dim=-1)
        to_add_feat_b1 = torch.cat((second_feature_pool, to_add_feat_b1), dim=-1)
